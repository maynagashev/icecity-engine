...
function sync_tree($unset_after = 1) {
  ...
  $not_exist_parents = array_diff($parents, $ids);
  ...
  $counts = array();
  $db->q("SELECT {$this->tree_item_field}, count(*) as c FROM {$sv->t[$this->tree_item_table]} GROUP BY {$this->tree_item_field}");
  while($d = $db->f()) {
  ...
  foreach($tree as $d) {
    ...
    $count = (isset($counts[$d['id']])) ? $counts[$d['id']] : 0;
    
    // count_all
    $count_all = $d['count']; // первоначальное значение - свое
    if (isset($childs_ids_all[$d['id']])) {
      foreach ($childs_ids_all[$d['id']] as $child_id) {
        if (isset($counts[$child_id])) {
          $count_all += $counts[$child_id];
        }
      }
    }
