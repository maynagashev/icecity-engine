...
function get_im_src($path, $update_c_format = 1) {  
  ...
  switch ($ext)
	...
	if ($ns) {
	  $this->err = 1;
		$this->errm[] = "Не поддерживаемый тип файла <b>{$ext}</b> в {$path}";
	}
	elseif ($im_src==false) {
	  $this->err = 1;
	  $this->errm[] = "Не удалось прочитать файл.";	  
	}
	else {
	 $ret = $im_src;
	}
...
function wh_resize($path, $target="") { 
  ...
  if (!file_exists($path)) {
    $errm[] = __FUNCTION__." - source file not exists: {$path}";
    $err = 1;
  }
...
function watermark($src_path, $wm_path, $margin = 10) {
  ...
  return $this->save_results($im, $src_path); 
}
